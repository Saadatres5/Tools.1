      ...
      <div className="flex gap-2">
        {Object.keys(ESSAY_TYPES).map(t=>(
          <button key={t} onClick={()=>setType(t)}
            className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${type===t?"bg-teal-600":"bg-gray-50 hover:bg-gray-100"}`}>{t}</button>
        ))}
      </div>
      <input value={topic} onChange={e=>setTopic(e.target.value)} onKeyDown={e=>e.key==="Enter"&&generate()}
        placeholder="e.g. climate change, social media, artificial intelligence"
        className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-800 placeholder-gray-400 outline-none focus:border-teal-500/50"/>
      ...
      {result && (
        <div>
          <div className="flex justify-between mb-2">
            <label className="text-xs text-gray-400">~{wc} words</label>
