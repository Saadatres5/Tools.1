    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {subjects.map(s=>(
          <button key={s} onClick={()=>setSubject(s)}
            className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-colors ${subject===s?"bg-cyan-600":"bg-gray-50 hover:bg-gray-100"}`}>{s}</button>
        ))}
      </div>
      <textarea rows={3} value={question} onChange={e=>setQuestion(e.target.value)}
        placeholder="Describe your homework question or topic..."
        className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-800 placeholder-gray-400 outline-none focus:border-cyan-500/50 resize-none"/>
      <div className="space-y-3">
        ...
        <div className="p-4 rounded-xl bg-gray-50 border border-gray-200">
          <h3 className="font-semibold text-xs text-gray-8000 mb-2">💡 General Study Tips</h3>
          <ul className="space-y-1">
            {QUICK_TIPS.map((t,i)=>(
              <li key={i} className="text-xs text-gray-8000 flex gap-2"><span>•</span>{t}</li>
            ))}
          </ul>
        </div>
        {question.trim() && (
          <div className="p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
            <h3 className="font-semibold text-xs text-yellow-600 mb-2">Your Question</h3>
            <p className="text-sm text-gray-600">{question}</p>
            <p className="text-xs text-gray-400 mt-2">Apply the {subject} steps above to work through this systematically. For complex problems, consider asking your teacher or using Khan Academy (khanacademy.org) for guided explanations.</p>
          </div>
        )}
      </div>
