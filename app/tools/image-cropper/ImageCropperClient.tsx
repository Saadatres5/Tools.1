    ...
    <div className="space-y-4">
      {!file ? (
        ...
        <div className="space-y-4">
          ...
          {imgSrc && <img src={imgSrc} alt="preview" className="max-h-48 rounded-xl object-contain w-full bg-white/5" />}
          <div>
            <label className="block text-xs text-white/50 mb-2">Crop Ratio</label>
            <div className="flex gap-2 flex-wrap">
              {aspects.map(([l,v]) => (
                <button key={v} onClick={() => setAspect(v)}
                  className={`px-3 py-1.5 rounded-xl text-xs transition-colors ${aspect===v?"bg-blue-600":"bg-white/5 hover:bg-white/10"}`}>{l}</button>
              ))}
            </div>
          </div>
          {result ? (
            <div className="space-y-3">
              <p className="text-green-400 text-sm">✅ Cropped!</p>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={result} alt="cropped" className="max-h-48 rounded-xl object-contain w-full bg-white/5" />
