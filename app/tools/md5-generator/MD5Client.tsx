...
function md5(str: string): string {
  ...
  const bs = utf8.split("").map(c=>c.charCodeAt(0));
  bs.push(0x80);
  while(bs.length%64!==56)bs.push(0);
  const l=str.length*8;
  bs.push(l&0xFF,(l>>8)&0xFF,(l>>16)&0xFF,(l>>24)&0xFF,0,0,0,0);
  const words:number[]=[];
  for(let i=0;i<bs.length;i+=4)words.push(bs[i]|(bs[i+1]<<8)|(bs[i+2]<<16)|(bs[i+3]<<24));
...
export default function MD5Client() {
  ...
  const generate = () => { if(input) setHash(md5(input)); };
