...
export default function NoiseRemoverClient() {
  const [file,setFile]=useState<File|null>(null);
  const [status,setStatus]=useState("idle");
  return (
    <div className="space-y-4">
      {!file ? (
        <FileDropZone accept="audio/*" emoji="🔇" label="Drop your file here to remove background noise" onFiles={f=>setFile(f[0])}/>
      ) : (
        <div className="space-y-4">
          ...
          <div className="p-5 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-center">
            ...
            <p className="text-gray-8000 text-sm mb-4">Ready to remove background noise.<br/>Full processing requires server-side integration.</p>
