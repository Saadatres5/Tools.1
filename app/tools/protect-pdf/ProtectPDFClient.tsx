    ...
    <div className="space-y-4">
      {!file?<FileDropZone accept=".pdf,application/pdf" emoji="🔐" label="Drop your PDF here" onFiles={f=>{setFile(f[0]);setStatus("idle");}}/>:(
        <div className="space-y-4">
          ...
          <div><label className="block text-xs text-white/50 mb-1">Password</label>
            <div className="relative">
              <input type={show?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Enter a strong password"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 pr-12 text-sm text-white outline-none focus:border-blue-500/50"/>
              <button onClick={()=>setShow(s=>!s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60 text-xs">{show?"Hide":"Show"}</button>
            </div>
          </div>
          {status==="done"?<div className="p-4 rounded-2xl bg-green-500/10 border border-green-500/20 space-y-3">
            <p className="text-green-400 font-semibold">✅ PDF protected!</p>
          ...
          </div>:status==="error"?<div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">❌ Failed.</div>:
