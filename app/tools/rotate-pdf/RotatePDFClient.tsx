        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {([90,180,270] as const).map(a=>(
              <button key={a} onClick={()=>setAngle(a)} className={`py-4 rounded-2xl text-center transition-colors border ${angle===a?"bg-blue-600 border-blue-500":"bg-white/5 border-white/10 hover:bg-white/10"}`}>
          ...
          {status==="done"&&<div className="p-4 rounded-2xl bg-green-500/10 border border-green-500/20 space-y-3">
            <p className="text-green-400 font-semibold">✅ Rotated {angle}°!</p>
          ...
          {status==="error"&&<div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">❌ Failed to rotate.</div>}
