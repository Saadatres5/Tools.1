      ...
      {!file?<FileDropZone accept="video/*" emoji="🎬" label="Drop your video here" hint="MP4, MOV, AVI, WEBM supported" onFiles={handleFile}/>:(
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={reset} className="text-gray-400 hover:text-gray-8000 text-sm ml-3">✕</button>
          </div>
          <video ref={videoRef} src={url} controls onLoadedMetadata={onLoad} className="w-full rounded-xl bg-black max-h-48"/>
          {duration>0&&(
            <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 space-y-3">
              <p className="text-xs text-gray-400">Duration: {fmt(duration)} • Set trim points below</p>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs text-gray-8000 mb-1">Start time (seconds)</label>
                  <input type="number" value={start} min="0" max={duration} step="0.1" onChange={e=>setStart(e.target.value)}
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 outline-none"/></div>
                <div><label className="block text-xs text-gray-8000 mb-1">End time (seconds)</label>
                  <input type="number" value={end} min="0" max={duration} step="0.1" onChange={e=>setEnd(e.target.value)}
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 outline-none"/></div>
              </div>
              <p className="text-xs text-gray-400">⚠️ Browser-based video trimming is limited. For full FFmpeg processing, a server-side tool is required. You can preview the trim range above using the video controls.</p>
