    ...
    <div className="space-y-4">
      {!file?<FileDropZone accept=".pdf,application/pdf" emoji="💧" label="Drop your PDF here" onFiles={f=>{setFile(f[0]);setStatus("idle");}}/>:(
        <div className="space-y-4">
          ...
          <div><label className="block text-xs text-white/50 mb-1">Watermark Text</label>
          ...
          <div><div className="flex justify-between mb-1"><label className="text-sm text-white/60">Opacity</label><span className="text-blue-400 text-sm font-medium">{opacity}%</span></div>
          ...
          {status==="done"?<div className="p-4 rounded-2xl bg-green-500/10 border border-green-500/20 space-y-3">
            <p className="text-green-400 font-semibold">✅ Watermark added!</p>
          ...
          </div>:status==="error"?<div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">❌ Failed to add watermark.</div>:
